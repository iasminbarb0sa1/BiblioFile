...
app.post("/api/livros", async (req, res) => {
  try {
    ...
    const [result] = await pool.query(
      "INSERT INTO livros (titulo, autor, genero, paginas, tempoLeitura, nota, comentario) VALUES (?, ?, ?, ?, ?, ?, ?)",
      [titulo, autor, genero, paginas, tempoLeitura, nota, comentario]
    );

    const [novoLivro] = await pool.query("SELECT * FROM livros WHERE id = ?", [result.insertId]);
